...
function connectModbus() {
  ...
  // Update the PLC connection configuration to use environment variables
  const plcHost = process.env.PLC_HOST || 'python-app';
  const plcPort = parseInt(process.env.PLC_PORT || '502');

  console.log(`[Attempt ${connectionAttempts}] Connecting to PLC at ${plcHost}:${plcPort}...`);
...
app.get('/plc/process-data', async (req, res) => {
  try {
    ...
    // Fix: Use consistent property access (.response.body instead of .response._body)
    const temp = tempResult.response.body.valuesAsArray[0] / 10;
    const pressure = pressureResult.response.body.valuesAsArray[0] / 100;
    const flow = flowResult.response.body.valuesAsArray[0] / 10;
    const pumpStatus = statusResult.response.body.valuesAsArray[0];
    ...
    // Fix: Pass object with required properties to calculateNPSHa
...
app.use((req, res, next) => {
  res.setHeader(
    ...
    "default-src 'self'; img-src 'self' data:; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline'; connect-src 'self'"
  );
  next();
});

// For any routes not matched by static files, serve index.html
app.use('/', (req, res) => {
